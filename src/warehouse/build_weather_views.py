...
SELECT
    ...
    COUNT(*) FILTER
    (
        WHERE overall_risk_level = 'UNKNOWN'
    ) AS unknown_hours,
...
CREATE_CRITICAL_PERIODS_VIEW_SQL = """
CREATE OR REPLACE VIEW vw_offshore_critical_periods AS
SELECT
    asset_id,
    asset_name,
    asset_type,
...
WHERE overall_risk_level IN ('AMBER', 'RED');
"""


def build_weather_views() -> None:
    logger.info(
        "Creating operational weather analytical views"
    )
    ...
    logger.success(
        "Operational weather analytical views created"
    )
